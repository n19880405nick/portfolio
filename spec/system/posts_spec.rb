...
describe '投稿', type: :system do
  ...
  #     end
  #     it '本文が表示されているか' do
  #       expect(page).to have_content post.contribution
  #     end
  #     it '「投稿を編集する」ボタンが表示されていないか' do
  #       expect(page).not_to have_link '', href: edit_post_path(post.id)
  #     end
  #     it 'いいねが表示されているか' do
  ...
  #       expect(page).not_to have_link '', href: post_comment_path(post, comment)
  #     end
  #   end
  # end
  describe '投稿詳細画面のテスト（ログイン）',js: true do
    let!(:comment){create(:comment, user: user, post: post)}
    before do
      sign_in user
      visit post_path(post.id)
    ...
    # context '表示の確認' do
      ...
      #   expect(page).to have_link '', href: post_comment_path(post, comment)
      # end
    # end
    context 'いいねのテスト' do
      it 'いいねを押したら１増える'  do
  ...
  #   context '表示の確認' do
  #     it '「投稿を編集する」ボタンが表示されていないか' do
  #       expect(page).not_to have_link '', href: edit_post_path(post)
  #     end
  #     it '「コメントを削除」ボタンが表示されていないか' do
  #       expect(page).not_to have_link '', href: post_comment_path(post, comment)
  #     end
